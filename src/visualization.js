...
function createSlider(selector, paramName, sliderParams, outputParams) {
  const valueProperty = (newValue) => {
    ...
    return newValue ? outputParams[paramName] = newValue : outputParams[paramName];
  }
  const lineBreak = document.createElement('br');
  const label = $('<label>').text(`${paramName} = ${valueProperty()}`);
  const slider = document.createElement('input');
  $(lineBreak).attr({class: 'auto-generated'});
  $(label).attr({class: 'auto-generated'}).css({ 'vertical-align': 'super', 'margin-left': 5 });;
...
function initSliders(_mainMethod, _lineSearchMethod) {
  ...
  Object.entries(lineSearchMethod.sliderParams).forEach(([key, params]) => {
    createSlider('#lineSearchMethodParameters', key, params, lineSearchMethod.params);
  })
}
...
function visualizeAlgorithm(plot, _points, color) {
  ...
  const circle = plot.svg.append('circle')
    .attr('class', 'dot')
    .attr('fill', color)
    .attr('r', 4)
  ...
  function transition() {
    ...
    circle.transition()
      ...
      .on('end', () => {if (points.length) transition()});
  }
  transition();
}
